...
/**
 * Find a job by its human-readable job number (e.g. 9888).
 * Returns the first matching job record or null if not found.
 *
 * @param {string|number} jobNumber  The sequential job number shown in JobNimbus UI
 * @returns {Object|null}
 */
async function getJobByNumber(jobNumber) {
  const num = parseInt(jobNumber, 10);
  try {
    // Try Elasticsearch-style filter first
    const { data } = await client().get('/jobs', {
      params: {
        filter: JSON.stringify({ must: [{ term: { number: num } }] }),
        size: 1,
      },
    });
    ...
    // Filter syntax may not be supported; fall through to scan
  }

  // Fallback: scan recent jobs for matching number
  let from = 0;
  const size = 100;
  while (true) {
    const { data } = await client().get('/jobs', {
      params: { size, from, sort: '-date_created' },
    });
    const results = data.results || data.data || [];
    if (results.length === 0) break;

    const match = results.find(j => String(j.number) === String(num) || j.job_number === String(num));
